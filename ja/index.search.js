var relearn_search_index = [
  ...
  {
    "content": "バージョン情報 目次 Version 1.0.0 Version 0.10.3 or earlier ",
    "description": "",
    "tags": null,
    "title": "バージョン情報",
  ...
  {
    "content": "CrySPY CrySPY 1.0.0以降はpipを使ってインストール可能になった．\npip install csp-cryspy pipでインストールすると，実行スクリプトのcryspyが自動的にPATHが通ったところにインストールされる． 下記コマンドでチェックできる．\nwhich cryspy cal_fingerprint (optional) cal_fingerprintプログラムは構造記述子を計算するためのもので，アルゴリズにベイズ最適化を使う場合は必須． CrySPY 1.0.0からはcal_fingerprintプログラムはCrySPY utilityに含まれている． コンパイルについてはこちらを参照：Instllation/CrySPY_utility/Compile cal_fingerprint．\n実行ファイルcal_fingerprintをPATHが通ったところに置く，もしくは下記のようにcryspy.inで実行ファイルのpathを指定する．\n[BO] fppath = /xxx/xxx/xxx/cal_fingerprint ",
    "description": "",
    "tags": null,
  ...
  {
    "content": "あらかじめ用意した構造のテキストデータをinit_struc_data.pklに変換するスクリプト． 入力のデフォルトはinit_POSCARS形式．オプションでPOSCARやcifファイルなどのシングル構造データも変換可能． 出力はinit_struc_data.pkl． すでに存在するinit_struc_data.pklに構造データを追加することもできる． 構造IDは考慮されず，新しく割り振られる． 原子数が異なる場合はエラーが出る．\ninit_struc_data.pklはCrySPYでシミュレーション開始時にロードすることが可能．\n使い方 usage: pos2pkl.py [-h] [-s [SINGLE ...]] [infile ...] positional arguments: infile input file: init_POSCARS optional arguments: -h, --help show this help message and exit -s [SINGLE ...], --single [SINGLE ...] input file: single structure file (POSCAR, cif) 例 init_POSCARS –\u003e init_struc_data.pkl CrySPYで構造生成後に出力されるinit_POSCARSをスパコン等の別のマシンに移すような場合に使える．複数のファイルを変換可能．\n$ python3 pos2pkl.py init_POSCARS Converted. The number of structures: 4 Save init_struc_data.pkl 複数の入力ファイルの場合．\n$ python3 pos2pkl.py init_POSCARS init_POSCARS2 init_POSCARS3 Converted. The number of structures: 12 Save init_struc_data.pkl カレントディレクトリにすでにinit_struc_data.pklが存在し，それに追加する場合．\n$ python3 pos2pkl.py init_POSCARS init_struc_data.pkl already exists. Append to init_struc_data.pkl? [y/n]: y Load init_struc_data. The number of structures: 12 Converted. The number of structures: 16 Save init_struc_data.pkl POSCAR or cif –\u003e init_struc_data.pkl POSCARファイルやcifファイル等の構造が一つだけのデータも変換可能．-s/--singleオプションが必要．\n$ python3 pos2pkl.py -s POSCAR test.cif Converted. The number of structures: 2 Save init_struc_data.pkl init_POSCARS, POSCAR –\u003e init_struc_data.pkl $ python3 pos2pkl.py init_POSCARS -s POSCAR Converted. The number of structures: 5 Save init_struc_data.pkl 警告 下記は間違い．init_POSCARSもシングル構造として取り扱われてしまう．\n$ python3 pos2pkl.py -s POSCAR init_POSCARS ",
    "description": "",
    "tags": null,
    "title": "pos2pkl.py",
    "uri": "/ja/cryspy_utility/pos2pkl/index.html"
  },
  {
  ...
  {
    "content": "機能 目次 バックアップ クリーン ジョブファイルのCrySPY_ID ",
    "description": "",
    "tags": null,
    "title": "機能",
  ...
  {
    "content": "CrySPY Utility ダウンロードに関してはこちらのページ： Installation/CrySPY utility．\n目次 Examples soiap_Si8_RS extract_struc.py pos2pkl.py kpt_check.py ",
    "description": "",
    "tags": null,
  ...
  },
  {
    ...
    "tags": null,
    "title": "kpt_check.py",
    "uri": "/ja/cryspy_utility/kpt_check/index.html"
  },
  {
